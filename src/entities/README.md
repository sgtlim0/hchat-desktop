...
FSD 아키텍처의 엔티티 레이어. 31개 Zustand 기반 상태 관리 스토어를 포함합니다. 모든 비즈니스 로직과 데이터 영속성이 이 레이어에서 처리됩니다.
...
├── context-manager/
│   └── context-manager.store.ts # 컨텍스트 매니저 (핀 메시지, 템플릿)
├── insights/
│   └── insights.store.ts        # AI 인사이트 + 크로스 세션 인사이트
├── plugins/
│   └── plugin.store.ts          # 플러그인 마켓플레이스
├── theme/
│   └── theme.store.ts           # 커스텀 테마 빌더
├── batch/
│   └── batch.store.ts           # 배치 프로세싱 큐
├── cache/
│   └── cache.store.ts           # 스마트 응답 캐싱
├── audit/
│   └── audit.store.ts           # 엔터프라이즈 감사 로그
├── dashboard/
│   └── dashboard.store.ts       # AI 대시보드 (위젯 그리드, 레이아웃)
└── workspace/
    └── workspace.store.ts       # 팀 워크스페이스 (멤버, 공유)
```
...
### ContextManagerStore (`context-manager.store.ts`)

컨텍스트 윈도우 시각화 및 관리. 핀 메시지, 자동 압축, 컨텍스트 템플릿(코딩/글쓰기/분석/일반).

| 상태 | 설명 |
|------|------|
| `pinnedMessages` | 고정 메시지 목록 |
| `selectedTemplate` | 선택된 컨텍스트 템플릿 |
| `autoCompression` | 자동 압축 활성화 여부 |
| `tokenUsage` | 토큰 사용량 (used/max) |

### InsightsStore (`insights.store.ts`)

AI 인사이트 대시보드 + 크로스 세션 인사이트 공유 스토어. 프롬프트 품질 점수, 모델 추천, 주간/월간 리포트, 세션 클러스터, 반복 패턴 분석.
...
| `clusters` | 세션 클러스터 |
| `patterns` | 반복 패턴 |
| `qualityScores` | 프롬프트 품질 점수 |
| `recommendations` | 모델 추천 |

### PluginStore (`plugin.store.ts`)

플러그인 마켓플레이스 상태 관리. 설치/제거/활성화, 권한 관리, 검색. 4개 기본 플러그인 내장.

| 상태 | 설명 |
|------|------|
| `plugins` | 플러그인 목록 |
| `selectedPluginId` | 선택된 플러그인 |
| `searchQuery` | 검색 쿼리 |

### ThemeStore (`theme.store.ts`)

커스텀 테마 빌더. CSS 변수 기반 색상 편집, 실시간 미리보기, 테마 활성화/복제.

| 상태 | 설명 |
|------|------|
| `themes` | 커스텀 테마 목록 |
| `selectedThemeId` | 선택된 테마 |

### BatchStore (`batch.store.ts`)

배치 프로세싱 큐. 대량 작업 병렬 처리, 3단계 우선순위(high/normal/low), 진행률 추적.

| 상태 | 설명 |
|------|------|
| `jobs` | 배치 작업 목록 |
| `selectedJobId` | 선택된 작업 |

### CacheStore (`cache.store.ts`)

스마트 응답 캐싱. 프롬프트 해시 기반 캐시, TTL 설정, 히트 카운트, 비용/토큰 절감 추적.
...
엔터프라이즈 감사 로그. 10가지 액션 타입, 날짜/액션/모델 필터링, CSV/JSON 내보내기.

| 상태 | 설명 |
|------|------|
| `entries` | 감사 로그 항목 |
| `filterAction` | 액션 필터 |
| `filterDateRange` | 날짜 범위 필터 |

### DashboardStore (`dashboard.store.ts`)

AI 대시보드 홈 위젯 그리드 관리. 레이아웃 CRUD, 위젯 추가/제거/순서 변경, 6종 위젯 타입.

| 상태 | 설명 |
|------|------|
| `layouts` | 대시보드 레이아웃 목록 |
| `activeLayoutId` | 현재 활성 레이아웃 |

주요 액션:
- `hydrate()` — IndexedDB에서 레이아웃 복원 (없으면 기본 6위젯 레이아웃 생성)
- `addLayout(name)` — 새 레이아웃 추가
- `deleteLayout(id)` — 레이아웃 삭제
- `addWidget(layoutId, type, title)` — 위젯 추가
- `removeWidget(layoutId, widgetId)` — 위젯 제거
- `reorderWidgets(layoutId, widgetIds)` — 위젯 순서 변경

### WorkspaceStore (`workspace.store.ts`)

팀 워크스페이스 관리. 워크스페이스 CRUD, 멤버 관리, 공유 프롬프트/지식베이스, 활동 피드.

| 상태 | 설명 |
|------|------|
| `workspaces` | 워크스페이스 목록 |
| `selectedWorkspaceId` | 선택된 워크스페이스 |

주요 액션:
- `hydrate()` — IndexedDB에서 워크스페이스 복원
- `createWorkspace(name, description)` — 워크스페이스 생성
- `addMember(workspaceId, name, email, role)` — 멤버 추가
- `sharePrompt(workspaceId, promptId)` — 프롬프트 공유
- `shareKnowledge(workspaceId, knowledgeId)` — 지식베이스 공유
- `addActivity(...)` — 활동 기록 (최대 50개 유지)
