...
## 🟤 Phase 9 — 스마트 UX & 멀티모달 확장 (예정)

### 9-1. AI 대시보드 홈 (2일)
- [ ] DashboardPage 신규 페이지 (HomeScreen 리팩토링)
- [ ] dashboard.store.ts Zustand 스토어
- [ ] 위젯 그리드 시스템 (드래그 리사이즈, 순서 변경)
- [ ] 위젯 6종: 최근 대화, 사용량 요약, 빠른 비서, 지식베이스 요약, 일정, 즐겨찾기
- [ ] 대시보드 레이아웃 localStorage 영속
- [ ] i18n 키 추가 (ko/en)

### 9-2. 멀티모달 채팅 확장 (2일)
- [ ] 이미지 입력 — Claude Vision, GPT-4o Vision 연동
- [ ] 카메라 캡처 — getUserMedia + 촬영 → 분석
- [ ] 오디오 입력 — Whisper API 연동 (음성 파일 → 텍스트)
- [ ] 파일 드래그앤드롭 개선 — 전역 드롭존, 미리보기 썸네일
- [ ] 멀티모달 메시지 렌더러 (이미지/오디오/PDF 인라인 표시)

### 9-3. 대화 분석 & 태그 자동화 (1일)
- [ ] AI 자동 태깅 — 대화 종료 시 LLM으로 태그 자동 추천
- [ ] 대화 감정 분석 — 긍정/부정/중립 색상 표시
- [ ] 스마트 제목 생성 — 첫 3회 메시지 기반 세션 제목 자동 생성
- [ ] 대화 유사도 추천 — "비슷한 대화" 사이드바 표시

### 9-4. 고급 프롬프트 에디터 (2일)
- [ ] AdvancedPromptEditor 위젯 (PromptInput 대체/확장)
- [ ] 멀티라인 확장 에디터 (Shift+Enter → 풀스크린)
- [ ] 변수 자동완성 (`{{` 입력 시 변수 팝업)
- [ ] 프롬프트 히스토리 (↑/↓ 이전 프롬프트 탐색)
- [ ] 프롬프트 스니펫 (/ 커맨드 → 라이브러리 삽입)
- [ ] 마크다운 미리보기 토글

### 9-5. 팀 워크스페이스 (3일)
- [ ] WorkspacePage 신규 페이지
- [ ] workspace.store.ts Zustand 스토어
- [ ] 워크스페이스 생성/참여 (이름, 아바타, 역할)
- [ ] 공유 프롬프트 라이브러리 (팀 공용 템플릿)
- [ ] 공유 지식베이스 (팀 문서 공유)
- [ ] 멤버 관리 (관리자/편집자/뷰어 3단 권한)
- [ ] 활동 피드 (최근 팀 활동 타임라인)
