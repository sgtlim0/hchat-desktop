...
> ✅ **전체 완료** (70/70 + Phase 1-5) | 마지막 업데이트: 2026-03-03
> 854 tests, 51 suites | 83% coverage | 빌드 성공
...
### 6-1. 프롬프트 체이닝 (1일)
- [ ] 순차 실행 체인 정의 UI
- [ ] 단계별 결과 → 다음 단계 입력 자동 연결
- [ ] 조건부 분기 (IF-THEN-ELSE)
- [ ] 실행 진행률 + 중간 결과 수정
- [ ] 기존 프롬프트 라이브러리 연동

### 6-2. 지식베이스 (2일)
- [ ] KnowledgeBasePage 신규 페이지
- [ ] knowledge.store.ts Zustand 스토어
...
### 6-3. 워크플로우 빌더 (3일)
- [ ] WorkflowBuilderPage 신규 페이지
- [ ] workflow.store.ts Zustand 스토어
- [ ] 블록 기반 비주얼 에디터 (드래그앤드롭)
- [ ] 사전 정의 블록: 프롬프트, 번역, 요약, 추출, 조건분기
- [ ] 트리거: 수동실행, 스케줄, 웹훅
- [ ] 변수 시스템: `{{output.previous}}` 체이닝
- [ ] 템플릿 갤러리 (일일보고서, 주간요약, 문서리뷰)

### 6-4. 실시간 협업 채팅 (2일)
- [ ] CollabRoomPage 신규 페이지
- [ ] collab.store.ts Zustand 스토어
- [ ] 룸 생성/참여 (링크 공유)
- [ ] 실시간 메시지 동기화 (WebRTC)
- [ ] 사용자별 커서/타이핑 표시
- [ ] 권한 관리 (호스트/참여자)
